...
**Created**: 2026-06-22  
**Status**: Draft  
**Input**: Extend the CDSS pipeline with (1) a router that distinguishes casual chat from research requests, (2) dynamic parallel document readers for both guideline sources and clinical trials, and (3) aggregation into a unified report. Readers spawn `min(document_count, MAX_READERS)` in parallel (MAX_READERS = 5 by default).

## Overview

Today every user message triggers the full research pipeline. Trials are fetched as thin API metadata only — eligibility criteria and interventions are never read. Guideline research already fans out up to five `SourceReader` agents in parallel, but trials do not.
...
4. **Given** one TrialReader fails (timeout), **When** the phase completes, **Then** other readers still contribute and a validation flag records the failure.

---

### User Story 3 - Unified document aggregation (Priority: P2)

Guideline source summaries and trial summaries are merged by a single **DocumentAggregator** agent (or two topic-specific aggregators feeding one synthesizer) into coherent "Standard of Care" and "Clinical Trials" sections before the final report.

**Why this priority**: Without aggregation, the synthesizer receives raw JSON blobs and quality is inconsistent.

**Independent Test**: Feed 3 source summaries + 2 trial summaries to aggregator mock; assert structured markdown sections out.

**Acceptance Scenarios**:

1. **Given** partial reader failures, **When** aggregation runs, **Then** the aggregator uses only successful summaries and notes gaps.
...
- **FR-002**: Research mode MUST run intake before any search or read phase.
- **FR-003**: Trial search MUST use ClinicalTrials.gov v2 via `curl_cffi` (TLS-safe client).
- **FR-004**: Trial deep-read MUST fetch per-study records including `eligibilityModule.eligibilityCriteria`.
- **FR-005**: Parallel readers MUST be capped at `MAX_READERS` (default 5) via semaphore; actual spawn count = `min(ranked_documents, MAX_READERS)`.
- **FR-006**: Ranking MUST occur before fan-out when candidates exceed `MAX_READERS`.
- **FR-007**: All agent spawns MUST go through `AgentFactory.spawn()` and emit trace events.
- **FR-008**: Reader failures MUST be isolated; one failure MUST NOT abort the run.
- **FR-009**: API and reader errors MUST surface as `validation_flags`, not silent empty results.
...
- Reading more than 5 documents in depth per run (mention-only for the rest)
...
- SC-004: Sidebar trace shows TrialReader children under TrialsCoordinator
